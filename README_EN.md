...
- **Gateway Layer**: FastAPI + WebSocket (Full-duplex communication)
- **Auditory Layer**: Silero VAD + pyannote.audio + Whisper (pywhispercpp)
- **Cognition Layer**: Apple MLX (mlx-lm)
- **TTS Layer**: MLX-Audio + Kokoro TTS (**True streaming output**)
- **Frontend**: Web Audio API + AudioWorklet

## Features

- ✅ **True Streaming TTS**: MLX-Audio + Kokoro generates audio chunk by chunk, no need to wait for complete text
- ✅ **Full-link Streaming**: Streaming processing from ASR → LLM → TTS
- ✅ **Full-duplex Communication**: Parallel WebSocket send/receive, supports barge-in and real-time interaction
- ✅ **Apple Silicon Optimized**: MLX framework fully utilizes M1/M2/M3/M4 chip performance
- ✅ **Chinese Support**: Kokoro model natively supports Chinese TTS
...
# TTS layer config (MLX-Audio + Kokoro)
tts:
  model: "mlx-community/Kokoro-82M-bf16"
  voice: "zf_xiaobei"  # Chinese female voice
  lang_code: "z"  # z = Chinese
  speed: 1.0
  sample_rate: 24000
```
...
│
├── tts/                        # TTS layer
│   └── mlx_tts.py            # MLX-Audio + Kokoro TTS engine (streaming)
│
├── core/                       # Core modules
